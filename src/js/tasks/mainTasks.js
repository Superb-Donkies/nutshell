...
function handleTasks (userId) {
    document.querySelector("#task-form").innerHTML = `<button id="add-task"><i class="far fa-plus-square"></i> New Task</button>`;
    DataManager.getTasks(userId)
        ...
        .then(() => {
            ...
            document.querySelector("#task-main").addEventListener("click", (e) => {
                ...
                if (e.target.id === "leave-task-form") {
                ...
                if (event.target.id === "save-task") {
                    ...
                    DataManager.saveTask(newTask)
                        .then(() => {
                            DataManager.getTasks(userId)
                                .then(response => {
                                    document.querySelector("#task-form").innerHTML = `<button id="add-task"><i class="far fa-plus-square"></i> Add Task</button>`;
                                    reloadPage(response);
                                })
                        })
                    }
